...
from ..open_clip.lora import apply_lora_to_model, get_lora_parameters, save_lora_weights, load_lora_weights
...
def main(args):
    ...
    if args.use_lora:
        if is_master(args):
            ...
            logging.info(f"LoRA rank: {args.lora_rank}, alpha: {args.lora_alpha}, dropout: {args.lora_dropout}")
        model, lora_modules = apply_lora_to_model(
            ...
            enable_vision=args.lora_enable_vision,
            enable_text=args.lora_enable_text,
        )
        if is_master(args):
            logging.info(f"Applied LoRA to {len(lora_modules)} modules")
        
        # Load pre-trained LoRA weights if specified
        if args.lora_weights is not None:
            if is_master(args):
                logging.info(f"Loading LoRA weights from {args.lora_weights}")
    ...
    if args.train_data or args.dataset_type == "synthetic":
        ...
        if opt.startswith('timm/'):
            ...
            if args.use_lora:
                ...
                if is_master(args):
                    logging.info(f"Optimizing {len(lora_params)} LoRA parameters")
        ...
        if args.horovod:
            ...
            optimizer = hvd.DistributedOptimizer(optimizer, named_parameters=named_parameters)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.save_logs:
            ...
            if completed_epoch == args.epochs or (
                ...
                # When using LoRA, only save LoRA weights to save disk space
                # Full model checkpoint is ~908MB, LoRA weights are only ~6MB
                if args.use_lora:
                    save_lora_weights(
                        model,
                        os.path.join(args.checkpoint_path, f"lora_epoch_{completed_epoch}.pt")
                    )
                else:
                    torch.save(
                        checkpoint_dict,
